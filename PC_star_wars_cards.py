    ...
    with sync_playwright() as p:
        ...
        try:
            for category in main_categories:
                ...
                if category.text_content()=='Star Wars Cards':
                    try:
                        # Click the category to open dropdown
                        
                        star_wars_cards_list = page.locator('#trading-cards > ul.menu-dropdown > li > a:has-text("Star Wars Cards")')
                        star_wars_cards_list_url = star_wars_cards_list.get_attribute('href')

                        new_page = browser.new_page()
                        new_page.goto(f"{base_url}{star_wars_cards_list_url}")

                        new_page.wait_for_selector('.home-box.all')
                        set_list = new_page.locator('.home-box.all ul li a').all()
                        
                        print('Getting the Set List for Star Wars Cards')
                        for set in set_list:
                            ...
                            if cards_data:
                                write_data_to_file(cards_data, f'Star Wars-Cards-{set_name}', "Star Wars Cards")
