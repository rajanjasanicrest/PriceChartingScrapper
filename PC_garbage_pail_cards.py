    ...
    with sync_playwright() as p:
        ...
        try:
            for category in main_categories:
                ...
                if category.text_content()=='Garbage Pail Cards':
                    try:
                        # Click the category to open dropdown
                        
                        garbage_pail_cards_list = page.locator('#trading-cards > ul.menu-dropdown > li > a:has-text("Garbage Pail Cards")')
                        garbage_pail_cards_list_url = garbage_pail_cards_list.get_attribute('href')

                        new_page = browser.new_page()
                        new_page.goto(f"{base_url}{garbage_pail_cards_list_url}")

                        new_page.wait_for_selector('.home-box.all')
                        set_list = new_page.locator('.home-box.all ul li a').all()
                        
                        print('Getting the Set List for Garbage Pail Cards')
                        for set in set_list:
                            ...
                            existing_data = []
                            try:
                                with open(f'data/{set_name}.json', 'r', encoding='utf-8') as f:
                            ...
                            for index, card_uri in enumerate(cards_list):
                                try:
                                    ...
                                    if index%50 == 0 and index!=0:
                                        ...
                                        with open(f'data/{set_name}.json', 'w', encoding='utf-8') as f:
                            ...
                            if cards_data:
                                write_data_to_file(cards_data, f'Garbage Pail-Cards-{set_name}', "Garbage Pail Cards")
