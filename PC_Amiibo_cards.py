    ...
    with sync_playwright() as p:
        ...
        try:
            for category in main_categories:
                ...
                if category.text_content()=='Amiibo Cards':
                    try:
                        # Click the category to open dropdown
                        
                        amiibo_cards_list = page.locator('#trading-cards > ul.menu-dropdown > li > a:has-text("Amiibo Cards")')
                        amiibo_cards_list_url = amiibo_cards_list.get_attribute('href')


                        existing_data = []
                        try:
                            with open(f'data/amiibo_cards.json', 'r', encoding='utf-8') as f:
                                existing_data = json.load(f)
                        except Exception as e:
                            print(e)
                            print('file doesnot exist')

                        print(f'Getting Cards List for set Amiibo cards')

                        cards_list = get_cards_list(amiibo_cards_list_url)
                        print(f"Number of cards to save: {len(cards_list)} for Amiibo Cards")
                        set_cards_list = []
                        ...
                        for index, card_uri in enumerate(cards_list):
                            try:
                                print(f"scrapping card {index} from Amiiibo Cards")
                                ...
                                if index%50 == 0 and index!=0:
                                    ...
                                    with open(f'data/amiibo_cards.json', 'w', encoding='utf-8') as f:
                        ...
                        if cards_data:
                            write_data_to_file(cards_data, f'Amiibo-Cards', "Amiibo Cards")
