    ...
    with sync_playwright() as p:
        ...
        try:
            for category in main_categories:
                ...
                if category.text_content()=='Pokemon Cards':
                    try:
                        # Click the category to open dropdown
                        
                        pokemon_cards_list = page.locator('#trading-cards > ul.menu-dropdown > li > a:has-text("Pokemon Cards")')
                        pokemon_cards_list_url = pokemon_cards_list.get_attribute('href')

                        new_page = browser.new_page()
                        new_page.goto(f"{base_url}{pokemon_cards_list_url}")

                        new_page.wait_for_selector('.home-box.all')
                        set_list = new_page.locator('.home-box.all ul li a').all()
                        
                        print('Getting the Set List for Pokemon Cards')
                        for set in set_list:
                            ...
                            if cards_data:
                                write_data_to_file(cards_data, f'Pokemon-Cards-{set_name}', "Pokemon Cards")
